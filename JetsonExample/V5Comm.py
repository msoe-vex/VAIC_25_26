...
class V5SerialComms:
    ...
    def __handle_message(self, header: str, body: str):
        try:
        ...
        while self.__started:  # Continue running while the thread is started
            ...
            try:
                ...
                while self.__started:  # Continue reading while thread is started
                    ...
                    # Invoke each callback on its own lightweight thread so serial reads remain responsive.
                    if self.__handler and (header or body):
                        callback_thread = threading.Thread(
                            target=self.__handle_message,
                            args=(header, body),
                            daemon=True,
                        )
